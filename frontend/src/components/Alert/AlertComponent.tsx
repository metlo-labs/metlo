...
import { Badge, Box, Heading, HStack, VStack, Button, useDisclosure, Modal, ModalOverlay, ModalHeader, ModalContent, ModalCloseButton, ModalBody, ModalFooter } from "@chakra-ui/react"
...
export const AlertComponent: React.FC<AlertComponentProps> = ({ alert, handleUpdateAlert, updating }) => {
      ...
      >
        <HStack px="4" w="full" justifyContent="space-between">
          <HStack>
            ...
            {alert.status === Status.RESOLVED && <FiCheckCircle fontSize="20px" color="green" />}
          </HStack>
        ...
        <HStack alignSelf="flex-end" px="4">
          {alert.status !== Status.RESOLVED && <Button
            isLoading={updating}
            onClick={() => handleUpdateAlert(alert.uuid, { updateType: alert.status === Status.IGNORED ? UpdateAlertType.UNIGNORE : UpdateAlertType.IGNORE })}
            leftIcon={alert.status === Status.OPEN ? <RiEyeOffFill /> : <RiEyeFill />}
            border="1px"
          >
      ...
      <Modal size="6xl" isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody maxH="700px" overflowY="auto">
            {alert &&<AlertDetail alert={alert} resolutionMessage={resolutionMessage} setResolutionMessage={setResolutionMessage} />}
          </ModalBody>
          {alert.status !== Status.IGNORED && <ModalFooter>
            {alert.status !== Status.RESOLVED ? (
              <Button
                isLoading={updating}
                leftIcon={<FiCheckCircle />}
                colorScheme="green"
                onClick={() => handleUpdateAlert(alert.uuid,{ updateType: UpdateAlertType.RESOLVE, resolutionMessage })}
              >
                Resolve
              </Button>
            ) : (
              <Button
                ...
                colorScheme="red"
                onClick={() => handleUpdateAlert(alert.uuid, { updateType: UpdateAlertType.UNRESOLVE })}
              >
                Unresolve
              </Button>
            )}
