...
use metlo_pcap::tcp_assembly::{clean_map, process_packet};
...
pub struct ParsedRequest {
    ...
    //pub headers: &'static mut [Header<'static>],
}
...
lazy_static! {
    ...
    static ref REQUEST_MAP: Mutex<HashMap<u64, MapPayload>> = {
        let m = HashMap::new();
        Mutex::new(m)
    };
}
...
fn run_payload(key_hash: u64, conn: &mut TcpConnection) {
            ...
            Ok(status) if status.is_complete() => {
                ...
                if req.method.is_some() {
                    ...
                    if let Some(resp_entry) = req_map.get(&resp_key_hash) {
                        // processTrace with this request and the response buffer from the map
                        /*println!(
                            "Found both payloads in request; Req Hash {}; Resp Hash {}",
                            key_hash, resp_key_hash
                        );*/
                    } else {
                        req_map.insert(
                            key_hash,
                            MapPayload {
                                buffer: conn.payload_buffer[0..bytes_parsed].to_vec(),
                    ...
                    if let Some(req_entry) = req_map.get(&req_key_hash) {
                        // processTrace with this response and the request buffer from the map
                        /*println!(
                            "Found both payloads in response; Req Hash {}; Resp Hash {}",
                            req_key_hash, key_hash
                        );*/
                    } else {
                        req_map.insert(
                            key_hash,
    ...
    if let Some(v) = val {
        ...
        if curr_seq_no == v.next_seq {
            ...
            for _i in 0..v.buffer.len() {
                println!(
                    "Trying buffer; Buffer Len {} {:?}; Current {} Looking for {}",
                    v.buffer.len(),
                    v.buffer.keys(),
                    curr_seq_no,
                    next_seq_no
                ...
                println!("Found buffer {}", remove_seq_no);
            ...
            println!(
                "out of order {}:{}->{}:{} for Hash {}, looking for {}, got {}, Is Start {}, Is End {}, data len {}; In buffer? {:?}; Buffer Len {}",
                ip_header.source_addr,
        tcp_header.source_port,
        ip_header.dest_addr,
        tcp_header.dest_port,key, v.next_seq, curr_seq_no, tcp_header.flag_syn, tcp_header.flag_fin || tcp_header.flag_rst, byte_len, v.buffer.keys(), v.buffer.len(),
            );
            if v.buffer.len() > 100 {
