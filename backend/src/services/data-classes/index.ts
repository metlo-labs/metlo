...
export async function cleanupStoredDataClasses(ctx: MetloContext, currentConfigResp: MetloConfigResp, newConfigResp: string) {
    ...
    const jsConfigCurrent = (jsyaml.load(currentConfig.configString) as object || {})
    ...
    try {
        ...
        const newDFs = DFs.map((DF) => {
            ...
            DF.falsePositives = DF.falsePositives.filter(dc => !dataClassesDiff.has(dc))
            DF.scannerIdentified = DF.scannerIdentified.filter(dc => !dataClassesDiff.has(dc))
            return DF
        })

        await getEntityManager(ctx, queryRunner).saveList(
            newDFs,
        )
...
export async function ensureValidCustomDataClasses(ctx: MetloContext, newConfigResp: string) {
        ...
        for (const configName of Object.keys(newConfig["sensitiveData"])) {
            if (existingClasses.includes(configName)) {
                return {
                    ...
                    err: new Error(`Custom class name matches existing class name ${configName}`)
                }
            }
        }
    }
