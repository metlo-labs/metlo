  ...
  static async generateOpenApiSpec() {
    ...
    Object.keys(hostMap).map(async (host) => {
      ...
      for (let i = 0; i < endpoints.length; i++) {
        ...
        endpoint.openapiSpec = newSpec;
      }
      newSpec.spec = JSON.stringify(openApiSpec);
      newSpec.isAutoGenerated = true;
      await openApiSpecRepository.save(newSpec);
