...
import { getWordList } from "services/metlo-config"
...
const generateEndpointsFromTraces = async (
  ...
  try {
    ...
    const wordSet = await getWordList(ctx)
    while (traces && traces?.length > 0) {
      ...
      for (let i = 0; i < traces.length; i++) {
        ...
        if (
          apiEndpoint &&
          !skipAutoGeneratedMatch(apiEndpoint, trace.path, wordSet)
        ) {
          ...
          if (!found) {
            ...
            for (let j = 0; j < pathTokens.length; j++) {
              ...
              if (tokenString === "/") {
                ...
                if (isSuspectedParamater(tokenString, wordSet)) {
