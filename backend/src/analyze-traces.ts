...
import { AnalysisType, RiskScore } from "@common/enums"
...
import { analyze as analyzeV2, analyzePartial } from "services/analyze/v2"
  ...
  if (pathRegex.length > 0) {
    ...
    try {
      ...
      await analyzeFunc(
        ...
        queryRunner,
        true,
        analysisType === AnalysisType.PARTIAL ? true : false,
...
const analyzePartialTraces = async (task: {
  ctx: MetloContext
  partialTraces: QueuedApiTrace[]
  version: number
  hasValidEnterpriseLicense: boolean
}) => {
  const { ctx, partialTraces, version, hasValidEnterpriseLicense } = task
  await getDataSource()
  let queryRunner = await getQueryRunner()
  const customWords = await getCustomWordsCached(ctx)
  for (const trace of partialTraces) {
    try {
      ...
      for (const traceItem of traces) {
        const start = performance.now()
        const apiEndpoint = await getEndpoint({ trace: traceItem, ctx })
        traceItem.createdAt = new Date(traceItem.createdAt)
        if (
          apiEndpoint &&
      ...
      mlog
        .withErr(err)
        .error("Encountered error while analyzing partial traces")
    }
  }
}
...
const runTask = async (task: { type: string; task: any }) => {
  ...
  } else if (task.type == "analyze_partial_traces") {
    await analyzePartialTraces(task.task)
