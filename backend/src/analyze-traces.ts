...
import { endpointAddNumberParams, endpointUpdateDates, isSuspectedParamater, skipAutoGeneratedMatch } from "utils"
...
SELECT
  ...
  array_agg(to_json(field. *)) as "dataFields"
FROM
  "api_endpoint" endpoint
  LEFT JOIN "open_api_spec" spec ON endpoint."openapiSpecName" = spec.name
  LEFT JOIN "data_field" field ON endpoint.uuid = field."apiEndpointUuid"
WHERE
  $1 ~ "pathRegex"
  AND method = $2
  AND host = $3
  ...
  while (true) {
    try {
      ...
      if (trace) {
        ...
        const apiEndpoint: ApiEndpoint = (await queryRunner.query(GET_ENDPOINT_QUERY, [trace.path, trace.method, trace.host]))?.[0]
