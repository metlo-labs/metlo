...
export const runAssertion = (
    ...
    if (assertion.type == AssertionType.enum.EQ) {
        ...
        if ((typeof assertion.val).toLowerCase() === "string") {
            if (assertionValue === executeScript(assertion.val as string, response, ctx))
                return true
        } else {
            throw new Error(`Script must be of type string. Received script of type ${typeof assertion.val}`)
        }
        return false
    }
    return false
